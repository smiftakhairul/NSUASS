				...
				<div class="widget-box">
					...
					<div class="widget-content nopadding">
						<form action="{{ route('appointments.store') }}" method="POST" class="form-horizontal">
							...
							@if(Session::has('appointmentRequestNotSent'))
							<div class="alert alert-danger">{!! Session::get('appointmentRequestNotSent') !!}</div>
							@endif
									...
									<select name="hostId" class="span6 hostsList" required>
										...
										@if(!$hosts->isEmpty())
											@foreach($hosts as $host)
												@if(old('hostId')==$host->id)
												<option selected value="{{ $host->id }}">{{ $host->name }}( {{ $host->hostDesignation->designation->name }} )</option>
												@else
